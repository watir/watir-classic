...
class WinClicker
    ...
    def setFileRequesterFileName( textToSet, title,  tryCount = 3 )
      	for i in (1..tryCount)
            # first set the Choose File Window to be active
            hWnd = getWindowHandle("#{title}" )
            
            if hWnd != -1
               puts "window handle is : hWnd" 
    ...
    def setFileRequesterFileName_newProcess ( textToSet , title = "Choose file")
        myapp = "ruby #{@path_to_clicker}/setFileDialog.rb #{textToSet} \"#{title}\""
    ...
    def clickJSDialog_NewProcess(button = "OK" )
        ...
        myapp = "ruby #{@path_to_clicker}/clickJSDialog.rb #{button}"
        puts "Starting win clicker in a new process. Looking for button #{button}"
        puts "Starting app: #{myapp}"
        winsystem( "start #{myapp}" )
